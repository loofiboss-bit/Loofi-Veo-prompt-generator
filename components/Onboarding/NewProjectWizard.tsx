...
import { PROJECT_TEMPLATES, ProjectTemplate } from '../../config/projectTemplates';
...
const NewProjectWizard: React.FC<NewProjectWizardProps> = ({ onSelectTemplate, onClose, isOpen }) => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
                    {PROJECT_TEMPLATES.map((template) => (
                        ...
                        >
                            ...
                            <div className={`absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity duration-500 bg-gradient-to-br ${template.gradient}`} />
                            
                            <div className={`w-12 h-12 rounded-xl mb-6 flex items-center justify-center bg-gradient-to-br ${template.gradient} shadow-lg`}>
                            ...
                            <div className="mt-auto pt-6 flex items-center text-xs font-semibold text-slate-500 group-hover:text-cyan-400">
                                ...
                                <Icon name="arrow-right" className="w-3 h-3 ml-2 transform group-hover:translate-x-1 transition-transform" />
                            </div>
                        </button>
                    ))}
                </div>
