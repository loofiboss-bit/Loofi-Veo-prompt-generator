...
import AutocompleteMenu, { AutocompleteItem } from '@shared/components/AutocompleteMenu';
...
const TextAreaInput = forwardRef<HTMLTextAreaElement, TextAreaInputProps>(
  (
    ...
    const [triggerMatch, setTriggerMatch] = useState<{ index: number; length: number } | null>(
      null,
    );
        ...
        if (triggerChar === '@') {
          results = characterBank
            ...
            .map((c) => ({
              ...
              label: c.name,
              description:
                `${c.attributes.age} ${c.attributes.gender}, ${c.wardrobe || ''}`.substring(0, 50) +
                '...',
              type: 'character',
            }));
        } else if (triggerChar === '#') {
      ...
      return parts.map((part, i) => {
        if (part.match(/^\{\{[A-Z0-9_]+\}\}$/)) {
          ...
          return (
            <span
              key={i}
              className={`inline-block rounded px-0.5 mx-0.5 ${exists ? 'bg-cyan-900/50 text-cyan-200 border border-cyan-500/30' : 'bg-red-900/50 text-red-300 border border-red-500/30'}`}
            >
              {part}
            </span>
          );
    ...
    return (
      <div className={`group relative ${className}`}>
        <div className="flex justify-between items-center mb-2">
          <label
            htmlFor={id}
            className="flex-grow flex items-center space-x-2 text-xs font-semibold text-slate-400 uppercase tracking-wide group-focus-within:text-cyan-400 transition-colors"
          >
          ...
          {maxLength && (
            <span
              className={`text-[10px] font-medium transition-colors ${characterCount > maxLength ? 'text-red-400' : 'text-slate-600 group-focus-within:text-slate-500'}`}
            >
