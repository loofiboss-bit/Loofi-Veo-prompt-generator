...
const TutorialGuide: React.FC<TutorialGuideProps> = ({ isActive, steps, currentStepIndex, onNext, onPrev, onFinish, uiStrings }) => {
    ...
    const getTailClass = () => {
        ...
        switch (tooltipPosition) {
            case 'top': return `${base} left-1/2 -translate-x-1/2 bottom-[-16px] border-t-slate-800`;
            case 'bottom': return `${base} left-1/2 -translate-x-1/2 top-[-16px] border-b-slate-800`;
            case 'left': return `${base} top-1/2 -translate-y-1/2 right-[-16px] border-l-slate-800`;
            case 'right': return `${base} top-1/2 -translate-y-1/2 left-[-16px] border-r-slate-800`;
            default: return '';
        }
    };
