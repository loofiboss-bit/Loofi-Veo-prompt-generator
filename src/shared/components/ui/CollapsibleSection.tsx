    ...
    >
      ...
      >
        <div className="flex items-center gap-4">
            {stepNumber && (
                <div className={`flex-shrink-0 w-8 h-8 rounded-full border flex items-center justify-center text-sm font-bold transition-colors ${isOpen ? C.numberBg : 'bg-slate-800 text-slate-500 border-slate-700 group-hover:border-slate-600'}`}>
                    {stepNumber}
                </div>
            )}
            <h2 className={`text-lg font-semibold transition-colors ${isOpen ? 'text-slate-100' : 'text-slate-300 group-hover:text-slate-200'}`}>{title}</h2>
        </div>
        <div className={`p-2 rounded-full transition-all duration-300 ${isOpen ? 'bg-slate-800 rotate-180 text-slate-200' : 'text-slate-600 group-hover:text-slate-400'}`}>
