...
const StyleTunerModal: React.FC<StyleTunerModalProps> = ({ isOpen, onClose, onSave }) => {
    const [step, setStep] = useState<'input' | 'generating' | 'selection' | 'analyzing' | 'success'>('input');
            ...
            variations.forEach(async (prompt, index) => {
                try {
                    ...
                    setOptions(prev => prev.map(opt => 
                        opt.id === index ? { ...opt, imageUrl, loading: false } : opt
                    ));
                } catch (e) {
                    console.error("Failed to generate thumbnail", e);
                    setOptions(prev => prev.map(opt => 
                        opt.id === index ? { ...opt, loading: false } : opt
                    ));
                }
            });
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 border-b border-slate-700 flex justify-between items-center bg-slate-800/50">
                    <div>
                        ...
                        <p className="text-sm text-slate-400 mt-1">Discover your aesthetic through visual selection.</p>
                    </div>
                ...
                <div className="flex-grow p-8 overflow-y-auto flex flex-col items-center justify-center">
                    ...
                    {step === 'input' && (
                        <div className="w-full max-w-lg space-y-6 text-center animate-fade-in-up">
                            ...
                            <p className="text-slate-400">Enter a simple subject. We'll generate different stylistic interpretations for you to choose from.</p>
                    ...
                    {step === 'selection' && (
                        <div className="w-full h-full flex flex-col">
                            <h3 className="text-center text-slate-300 mb-6 font-medium">Select the image that best matches your vision:</h3>
                                ...
                                {options.map((opt) => (
                                    ...
                                    >
                                        {opt.loading ? (
                                            ...
                                            <img src={opt.imageUrl} alt="Style option" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                                        ) : (
                        ...
                        <div className="w-full max-w-md space-y-6 animate-fade-in-up">
                            ...
                            <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-700 space-y-3">
                                <div className="flex justify-between text-sm border-b border-slate-700/50 pb-2">
                                    ...
                                    <span className="text-cyan-300 font-medium">{extractedDNA.artStyle || 'Custom'}</span>
                                </div>
                                <div className="flex justify-between text-sm border-b border-slate-700/50 pb-2">
                                    <span className="text-slate-400">Lighting</span>
                                    <span className="text-cyan-300 font-medium">{extractedDNA.lightingStyle || 'N/A'}</span>
                                </div>
                                <div className="flex justify-between text-sm border-b border-slate-700/50 pb-2">
                                    <span className="text-slate-400">Colors</span>
                                    <span className="text-cyan-300 font-medium">{extractedDNA.colorPalette || 'N/A'}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-slate-400">Camera</span>
                                    <span className="text-cyan-300 font-medium">{extractedDNA.cameraMovement || 'N/A'}</span>
                                </div>
                            </div>
