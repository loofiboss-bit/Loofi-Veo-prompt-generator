...
import { apiExportService, type ExportFormat, type CodeSnippet } from '@core/services/apiExportService';
...
const ApiExportModal: React.FC<ApiExportModalProps> = ({ isOpen, onClose, entry, addToast }) => {
    ...
    const [activeTab, setActiveTab] = useState<'export' | 'snippets' | 'postman' | 'openapi'>('export');
                    ...
                    {activeTab === 'snippets' && (
                        <div className="space-y-4">
                            {codeSnippets.map((snippet, index) => (
                                <div key={index}>
                                    <div className="flex items-center justify-between mb-2">
                                        <label className="text-sm font-semibold text-slate-300">{snippet.description}</label>
                                        <button
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        onClick={() => handleDownload(postmanCollection, 'veo-studio-api.postman_collection.json')}
                                        className="px-3 py-1.5 bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-semibold rounded-lg transition-colors flex items-center gap-2"
                                    >
                                        <Icon name="download" className="w-3 h-3" />
                                        Download
