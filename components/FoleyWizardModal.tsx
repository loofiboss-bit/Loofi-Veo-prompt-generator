...
import { extractLastFrame, extractFramesFromVideo } from '../utils/videoUtils';
import { useAppStore } from '../store/useAppStore'; // For Timeline manipulation
...
const FoleyWizardModal: React.FC<FoleyWizardModalProps> = ({ 
    ...
    const handleMagicGenerateAll = async () => {
        ...
        for (const event of magicEvents) {
            ...
            setMagicEvents(prev => prev.map(e => e.id === event.id ? { ...e, status: 'generating' } : e));
            ...
            try {
                ...
                setMagicEvents(prev => prev.map(e => e.id === event.id ? { ...e, status: 'done', blob } : e));

            } catch (err) {
                setMagicEvents(prev => prev.map(e => e.id === event.id ? { ...e, status: 'error' } : e));
            }
        }
                        ...
                        <div className="space-y-6">
                            <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                                <p className="text-sm text-slate-300 mb-4">
                                    Analyze the full video clip to detect multiple audio events and place them precisely on the timeline.
                                </p>
                                ...
                                {magicEvents.length === 0 && (
                                    ...
                                    >
                                        {isMagicAnalyzing ? <Icon name="spinner" className="w-5 h-5 animate-spin" /> : <Icon name="video-analysis" className="w-5 h-5" />}
                                        Analyze Video
                                    </button>
                                )}
                            </div>
                            ...
                            {magicEvents.length > 0 && (
                                <div className="space-y-4">
                                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Detected Events</h3>
                                    <div className="space-y-2">
                                        {magicEvents.map((event) => (
                                            <div key={event.id} className="flex items-center gap-3 p-3 bg-slate-800 rounded-lg border border-slate-700">
                                                ...
                                                <div className="text-xs">
                                                    {event.status === 'pending' && <span className="text-slate-500">Wait</span>}
                                                    {event.status === 'generating' && <Icon name="spinner" className="w-4 h-4 text-yellow-400 animate-spin" />}
                                                    {event.status === 'done' && <Icon name="check" className="w-4 h-4 text-green-400" />}
                                                    {event.status === 'error' && <Icon name="alert-triangle" className="w-4 h-4 text-red-400" />}
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    >
                                        {isMagicGenerating ? <Icon name="spinner" className="w-5 h-5 animate-spin" /> : <Icon name="magic" className="w-5 h-5" />}
                                        Generate & Sync All
                                    </button>
                                </div>
                            )}
