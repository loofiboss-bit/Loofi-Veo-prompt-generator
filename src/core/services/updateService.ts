...
class UpdateService {
  ...
  async checkForUpdates(): Promise<UpdateStatus> {
    ...
    try {
      ...
      if (latestRelease) {
        const isNewer = this.compareVersions(latestRelease.version, this.CURRENT_VERSION);
   ...
   */
  private detectChannel(release: { tag_name: string }): ReleaseChannel {
  ...
  private getPlatform(): string {
    ...
    if (electron) {
      return electron.platform || 'linux';
    }
  ...
  private async electronDownload(url: string): Promise<void> {
    ...
    electron.onDownloadProgress?.((progress: number) => {
      this.status = {
        ...this.status,
