      ...
      {isError ? (
        ...
        <>
          ...
          <div className="flex space-x-1.5 mt-2">
            {stages.map((stage, i) => {
              const isActive =
                i === currentStageIndex || (status === 'Polling' && stage === 'Processing');
              const isDone = i < currentStageIndex || isComplete;
              return (
                <div
                  key={stage}
                  className={`w-2 h-2 rounded-full transition-all duration-500 ${isDone ? 'bg-cyan-500' : isActive ? 'bg-cyan-500/50 animate-pulse' : 'bg-slate-700'}`}
                />
              );
            })}
          </div>
...
const VideoGenerationStudio: React.FC<VideoGenerationStudioProps> = ({
  ...
  const aspectRatioOptions = getAspectRatios(language).filter((o) =>
    ['16:9', '9:16'].includes(o.value),
  );
  ...
  const handleGenerate = async () => {
    ...
    await videoGenerationService.startGeneration(
      ...
      undefined, // No initial image for standard generation
            ...
            {displayTasks.length === 0 ? (
              <div className="flex-grow bg-slate-950/80 rounded-xl border border-slate-800 flex items-center justify-center relative overflow-hidden shadow-inner p-8">
                <div className="text-center text-slate-600 flex flex-col items-center">
                  <Icon
                    name={activeTab === 'queue' ? 'clock' : 'film'}
              ...
              <div className={`grid ${gridCols} gap-4 h-full overflow-y-auto pr-2`}>
                {displayTasks.map((task) => (
                  <div
                    key={task.id}
                    className="flex flex-col bg-slate-950/80 rounded-xl border border-slate-800 overflow-hidden shadow-inner min-h-[250px] relative group"
                  >
                    ...
                    <div className="flex-grow relative flex items-center justify-center">
                      {task.videoUrl ? (
                        <video
                          src={task.videoUrl}
                          controls
                          autoPlay={displayTasks.length === 1}
                          loop
        ...
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-xl flex items-center justify-center z-[60] p-4">
          <div className="bg-slate-900 border border-slate-700 p-8 rounded-2xl max-w-lg text-center shadow-2xl">
            ...
            <p className="text-slate-300 mt-2 text-base leading-relaxed">
              Video generation with Veo 3.1 requires a valid API key associated with a project that
              has billing enabled.
            </p>
            <p className="text-slate-400 mt-4 mb-8 text-sm">
              Please select your key to continue. For more information, please see the{' '}
              <a
