...
class APIExportService {
     ...
     */
    generateCodeSnippets(entry: HistoryEntry, baseUrl: string = 'https://api.example.com'): CodeSnippet[] {
    ...
    async batchExport(
        ...
        try {
            const exports = await Promise.all(
                entries.map((entry) => this.exportPrompt(entry, options))
            );
