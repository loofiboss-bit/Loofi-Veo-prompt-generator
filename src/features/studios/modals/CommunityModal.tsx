...
const CommunityGallery: React.FC<CommunityModalProps> = ({ onImport }) => {
  ...
  const filteredDnas = dnas.filter(
    (dna) =>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredDnas.map((dna) => (
              <div
                key={dna.id}
                className="bg-slate-800/40 border border-slate-700 rounded-xl p-4 hover:border-cyan-500/30 transition-all group flex flex-col relative overflow-hidden"
              >
                {/* Header */}
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium transition-colors ${
                      likedIds.has(dna.id)
                        ...
                        : 'bg-slate-700/50 text-slate-400 hover:text-pink-400 hover:bg-pink-500/10'
                    }`}
                  >
                ...
                <div className="flex flex-wrap gap-1 mb-4 flex-grow content-start">
                  {dna.styleParams.artStyle && (
                    <span className="px-2 py-0.5 bg-cyan-900/30 text-cyan-300 border border-cyan-500/20 rounded text-[10px] truncate max-w-full">
                      {dna.styleParams.artStyle === 'Custom'
                        ? dna.styleParams.customArtStyle
                        : dna.styleParams.artStyle}
                    </span>
                  )}
