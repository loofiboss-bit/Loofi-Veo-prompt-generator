...
const MotionEditorPanel: React.FC<MotionEditorPanelProps> = ({ shot, onSave, onClose }) => {
    const [config, setConfig] = useState<MotionConfig>(shot.motionConfig || {
    ...
    if (!imageUrl) {
        return (
            <div className="bg-slate-900 p-6 rounded-lg text-center border border-slate-700">
                ...
                <button onClick={onClose} className="mt-4 px-4 py-2 bg-slate-800 text-white rounded">Close</button>
            </div>
        );
    }
