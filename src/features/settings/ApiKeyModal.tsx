  ...
  const content = (
    <div
      className={
        embedded
          ? ''
          : 'bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden'
      }
    >
      {/* Header */}
      {!embedded && (
      ...
      {/* Footer */}
      <div
        className={`flex items-center justify-between ${embedded ? 'pt-4' : 'p-5 border-t border-slate-700/50 bg-slate-800/30'}`}
      >
        <button
          onClick={handleClear}
