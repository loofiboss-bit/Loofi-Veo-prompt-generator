...
self.addEventListener('message', async (event) => {
    ...
    if (type === 'RENDER_JOB') {
        const { clips, outputName, filters, cropConfig, backgroundMusic, audioSettings, fonts } = payload;
        ...
        try {
            ...
            for (let i = 0; i < clips.length; i++) {
                ...
                if (clip.isTitle && clip.titleConfig) {
                    const titleImgData = await generateTitleCardImage(clip.titleConfig.text, clip.titleConfig);
                ...
                if (clip.colorGrade) {
                    ...
                    filterParts.push(`eq=contrast=${cg.contrast}:brightness=${cg.brightness}:saturation=${cg.saturation}:gamma_r=${cg.gamma_r||1}:gamma_g=${cg.gamma_g||1}:gamma_b=${cg.gamma_b||1}`);
                }
                ...
                // Audio Filtering
                if (clip.audioData) {
                    const vol = (clip.audioVolume !== undefined ? clip.audioVolume : 1.0) * (audioSettings?.volumes?.dialogue ?? 1.0);
                ...
                // Cleanup raw
                try { await instance.deleteFile(rawVidName); } catch(e){}
                try { if(clip.audioData) await instance.deleteFile(rawAudName); } catch(e){}
            ...
            for (let i = 0; i < processedClips.length - 1; i++) {
                ...
                if (transition.type === 'cut') {
                    ...
                    filterGraph.push(`${vPrev}[${i+1}:v]xfade=transition=${method}:duration=${transDur}:offset=${offset}[${nextLabelV}];`);
                    filterGraph.push(`${aPrev}[${i+1}:a]acrossfade=d=${transDur}:c1=tri:c2=tri[${nextLabelA}];`);
                }
            ...
            const stitchCmd = [
                ...inputArgs,
                '-filter_complex', filterGraph.join('').slice(0, -1), // remove trailing ;
                '-map', finalV,
                '-map', finalA,
            ...
            if (processedClips.length === 1 && !backgroundMusic) {
                 await instance.exec(['-i', processedClips[0].name, '-c:v', 'libx264', '-c:a', 'aac', outputName]);
            } else {
            ...
            for(const c of processedClips) try { await instance.deleteFile(c.name); } catch(e){}
