...
const Timeline: React.FC<TimelineProps> = ({ 
    timelineState, onClipUpdate, onSeek, duration, isRecording, 
    startVideoGeneration, onSelectClip, selectedClipId 
}) => {
    ...
    const handleFillGap = async (start: number, end: number) => {
        ...
        try {
            const contextText = sbShots.length > 0 ? (sbShots[0].action || "Scene context") : "Ambient scene";
        ...
        <div className="flex flex-col h-full bg-slate-950 select-none">
            ...
            <div className="flex-grow overflow-auto relative custom-scrollbar" onScroll={handleScroll} ref={containerRef}>
                <div className="relative" style={{ width: `${totalWidth + headerWidth}px` }}>
                    <div className={`absolute top-0 bottom-0 w-px z-50 pointer-events-none transition-colors ${isRecording ? 'bg-red-500' : 'bg-white'}`} style={{ left: `${(currentTime * zoomLevel) + headerWidth}px` }}>
                    ...
                    {tracks.map((track, trackIndex) => (
                        <TimelineTrackView
                            ...
                            zoomLevel={zoomLevel} duration={duration + 10} 
                            ...
                            >
                                <div className="flex gap-1 pointer-events-auto opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    {gap.trackId === 'video_main' && (
                                        ...
                                        >
                                            {fillingGapId === `${gap.start}-${gap.end}` ? <Icon name="spinner" className="w-4 h-4 animate-spin" /> : <Icon name="magic" className="w-4 h-4" />}
                                        </button>
                                    )}
                                </div>
