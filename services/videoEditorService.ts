...
import { VideoFilters, ClipTransition, CropConfig, TextOverlay, ColorGradeParams, MotionConfig, VisualizerConfig } from '../types';
...
export const generateProxy = async (sourceUrl: string): Promise<string> => {
    ...
    try {
        ...
        await instance.exec([
            '-i', inputName, '-vf', 'scale=-2:480', '-c:v', 'libx264',
            '-preset', 'ultrafast', '-b:v', '500k', '-c:a', 'aac', '-b:a', '96k', outputName
        ]);
...
export const stitchVideos = async (
    ...
    return new Promise((resolve, reject) => {
        ...
        }, [messageChannel.port2]); // Transferable if needed
    });
};
...
export const renderAudioVisualizer = async (
    ...
    const cmd = [
        ...
        '-map', '0:a', '-c:v', 'libx264', '-preset', 'ultrafast', '-r', String(fps), '-c:a', 'aac', '-b:a', '192k', '-shortest', outputName
    ];
    await instance.exec(cmd);
    const outData = await instance.readFile(outputName);
    const blob = new Blob([outData], { type: 'video/mp4' });
...
export const transcodeVideo = async (sourceUrl: string, profile: ExportProfile, onProgress?: (msg: string) => void): Promise<string> => {
    ...
    if (profile.container === 'gif') {
        ...
        cmd.push('-vf', 'fps=15,scale=480:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse', '-f', 'gif');
    } else {
