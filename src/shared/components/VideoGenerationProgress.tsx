          ...
          return (
            <React.Fragment key={stage.key}>
              <div className="flex flex-col items-center text-center w-20">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${
                    isCompleted
                      ...
                      : isActive
                        ? 'bg-slate-800 border-cyan-500 animate-pulse'
                ...
                >
                  {stageLabels[stage.key as keyof typeof stageLabels]}
                </p>
              </div>
              {index < STAGES.length - 1 && (
                <div
                  className={`flex-1 h-1 mt-5 mx-2 transition-colors duration-500 rounded-full ${isCompleted ? 'bg-cyan-500' : 'bg-slate-700'}`}
                ></div>
              )}
            </React.Fragment>
          );
    ...
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-lg flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900/70 backdrop-blur-xl w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-700/50 p-6 text-center">
        ...
        {detailedMessage && !generatedVideoUrl && (
          <p className="text-center text-slate-300 text-sm min-h-[1.25rem]">{detailedMessage}</p>
        )}
