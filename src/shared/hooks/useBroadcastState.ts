...
export function useBroadcastState<T>(
  initialState: T,
  debounceTimeout = 500,
): [T, SetBroadcastState<T>, boolean, () => void, () => void, boolean, boolean] {
  ...
  const setBroadcastState = useCallback<SetBroadcastState<T>>(
    (update, action) => {
      const now = Date.now();
      const isReplace = action === 'replace';

      // Calculate new state
      const newState = isReplace ? (update as T) : { ...stateRef.current, ...update };
