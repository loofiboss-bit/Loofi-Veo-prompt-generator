...
const InspectorPanel: React.FC<InspectorPanelProps> = ({ selectedClip, onUpdate, currentTime }) => {
  ...
  const PropertyRow: React.FC<{
    ...
    const keyframed = isKeyframed(propertyKey);
          ...
          <div className="flex items-center gap-2">
            ...
            {keyframed ? (
              <button
                ...
                className={`p-1 rounded hover:bg-slate-700 transition-colors text-cyan-400 ${!isWithinClip ? 'opacity-30 cursor-not-allowed' : ''}`}
                title="Toggle Keyframe"
                aria-label={`Toggle keyframe for ${label}`}
                aria-pressed="true"
              >
                <Icon name="keyframe-filled" className="w-3 h-3" />
              </button>
            ) : (
              <button
                onClick={() => toggleKeyframe(propertyKey)}
                disabled={!isWithinClip}
                className={`p-1 rounded hover:bg-slate-700 transition-colors text-slate-600 ${!isWithinClip ? 'opacity-30 cursor-not-allowed' : ''}`}
                title="Toggle Keyframe"
                aria-label={`Toggle keyframe for ${label}`}
                aria-pressed="false"
              >
                <Icon name="keyframe" className="w-3 h-3" />
              </button>
            )}
          </div>
        </div>
    ...
    <div className="w-80 h-full bg-slate-900 border-l border-slate-700 flex flex-col overflow-hidden animate-fade-in-up">
      ...
      <div className="p-4 border-b border-slate-800 bg-slate-800/30">
        <div className="flex items-center gap-2 mb-1">
          <div
            className={`w-2 h-2 rounded-full ${selectedClip.type === 'video' ? 'bg-cyan-500' : selectedClip.type === 'audio' ? 'bg-fuchsia-500' : 'bg-yellow-500'}`}
          ></div>
      ...
      >
        {activeSection === 'transform' ? (
          <button
            ...
            className="flex-1 py-3 text-[10px] font-semibold uppercase transition-colors text-cyan-400 border-b-2 border-cyan-400 bg-slate-800/50"
          >
            Transform
          </button>
        ) : (
          <button
            ...
            aria-selected="false"
            onClick={() => setActiveSection('transform')}
            className="flex-1 py-3 text-[10px] font-semibold uppercase transition-colors text-slate-500 hover:text-slate-300"
          >
            Transform
          </button>
        )}
        {selectedClip.type === 'video' &&
          (activeSection === 'effects' ? (
            <button
              ...
              className="flex-1 py-3 text-[10px] font-semibold uppercase transition-colors text-yellow-400 border-b-2 border-yellow-400 bg-slate-800/50"
            >
              Effects
            </button>
          ) : (
        ...
        {selectedClip.type === 'audio' &&
          (activeSection === 'audio' ? (
            <button
              ...
              className="flex-1 py-3 text-[10px] font-semibold uppercase transition-colors text-fuchsia-400 border-b-2 border-fuchsia-400 bg-slate-800/50"
            >
              Audio
            </button>
          ) : (
                ...
                <div className="p-3 space-y-3">
                  ...
                  {effect.type === 'color' && (
                    <>
                      <PropertyRow
                        label="Brightness"
                        value={(effect as ColorGradeEffect).brightness}
                        onChange={(v) => updateEffect(effect.id, { brightness: v })}
                        min={0}
                        max={2}
                        step={0.1}
                        propertyKey="color.brightness"
                      />
                      <PropertyRow
                        label="Contrast"
                        value={(effect as ColorGradeEffect).contrast}
                        onChange={(v) => updateEffect(effect.id, { contrast: v })}
                        min={0}
                        max={2}
                        step={0.1}
                        propertyKey="color.contrast"
                      />
                      <PropertyRow
                        label="Saturation"
                        value={(effect as ColorGradeEffect).saturation}
                        onChange={(v) => updateEffect(effect.id, { saturation: v })}
                        min={0}
                        max={2}
                        step={0.1}
                        propertyKey="color.saturation"
                      />
                    </>
                  )}
                    ...
                    <>
                      <div className="mb-3">
                        ...
                        <input
                          ...
                          className="w-full h-6 rounded cursor-pointer bg-slate-700 border-0"
                        />
                      </div>
                      <PropertyRow
                        label="Similarity"
                        value={(effect as ChromaKeyEffect).similarity}
                        onChange={(v) => updateEffect(effect.id, { similarity: v })}
                        min={0}
                        max={1}
                        step={0.01}
                        propertyKey="chroma.similarity"
                      />
                      <PropertyRow
                        label="Smoothness"
                        value={(effect as ChromaKeyEffect).smoothness}
                        onChange={(v) => updateEffect(effect.id, { smoothness: v })}
                        min={0}
                        max={1}
                        step={0.01}
                        propertyKey="chroma.smoothness"
                      />
                    </>
                  )}
                  ...
                  {effect.type === 'shake' && (
                    <>
                      <PropertyRow
                        label="Intensity"
                        value={(effect as CameraShakeEffect).intensity}
                        onChange={(v) => updateEffect(effect.id, { intensity: v })}
                        min={0}
                        max={1}
                        step={0.05}
                        propertyKey="shake.intensity"
                      />
                      <PropertyRow
                        label="Speed"
                        value={(effect as CameraShakeEffect).speed}
                        onChange={(v) => updateEffect(effect.id, { speed: v })}
                        min={0.1}
                        max={5}
                        step={0.1}
                        propertyKey="shake.speed"
                      />
                      <PropertyRow
                        label="Overscan"
                        value={(effect as CameraShakeEffect).scale}
                        onChange={(v) => updateEffect(effect.id, { scale: v })}
                        min={1}
                        max={1.5}
                        step={0.01}
                        propertyKey="shake.scale"
                      />
                    </>
                  )}
                </div>
