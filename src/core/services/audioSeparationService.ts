...
// Setup transformers env if we were to use it fully client-side later
...
export const separateStems = async (audioUrl: string): Promise<SeparationResult> => {
    try {
        ...
        const tempCtx = new (window.OfflineAudioContext || (window as any).webkitOfflineAudioContext)(1, 1, 44100);
