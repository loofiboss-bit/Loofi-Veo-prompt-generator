...
export type VariableCategory = 'character' | 'location' | 'time' | 'camera' | 'style' | 'custom';
...
export function extractVariablesFromState(promptState: any): Record<string, string> {
  ...
  if (promptState.characterEthnicity)
    variables.character_ethnicity = promptState.characterEthnicity;
...
export function insertVariable(
  ...
  const placeholder = defaultValue ? `{{${variableName}:${defaultValue}}}` : `{{${variableName}}}`;
 ...
 */
export function createCustomVariable(name: string, value: string, description?: string): Variable {
 ...
 * Merge variable sets
 */
export function mergeVariables(...variableSets: Record<string, string>[]): Record<string, string> {
