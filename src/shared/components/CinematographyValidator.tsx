...
const CinematographyValidator: React.FC<CinematographyValidatorProps> = ({ promptState, uiStrings, addToast }) => {
    ...
    const handleCheck = async () => {
        ...
        try {
            const validationResult = await geminiService.validateCinematography(promptState);
            setResult(validationResult);
            if (validationResult.isValid) {
                addToast("Cinematography choices look good.", 'success');
            } else {
        ...
        <div className="mt-6 border border-slate-700/50 rounded-xl bg-slate-900/30 overflow-hidden">
            <div className="p-4 border-b border-slate-700/50 flex justify-between items-center bg-slate-800/20">
                <h3 className="text-sm font-semibold text-slate-200 flex items-center gap-2">
                    <Icon name="video" className="w-4 h-4 text-cyan-400" />
                    Cinematography Logic Check
                </h3>
                <button
                    onClick={handleCheck}
                ...
                >
                    {isChecking ? <Icon name="spinner" className="w-3.5 h-3.5 animate-spin" /> : <Icon name="play" className="w-3.5 h-3.5" />}
                    <span>{uiStrings.cinematographyCheck?.runButton || "Check Cinematography"}</span>
                </button>
            </div>

            {result && (
                <div className={`p-4 border-l-4 transition-all duration-500 animate-fade-in-up ${result.isValid ? 'border-green-500/60 bg-green-900/10' : 'border-red-500/60 bg-red-900/10'}`}>
                    <div className="flex items-start gap-3">
                        <div className={`mt-0.5 p-1 rounded-full ${result.isValid ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                            <Icon name={result.isValid ? 'check' : 'alert-triangle'} className="w-4 h-4" />
                        </div>
                        <div>
                            <h4 className={`text-sm font-bold mb-1 ${result.isValid ? 'text-green-300' : 'text-red-300'}`}>
                                {result.isValid 
                            ...
                            {result.isValid ? (
                                <p className="text-xs text-green-200/80">
                                    {uiStrings.cinematographyCheck?.validMessage || "Lighting, optics, and camera choices are harmonious."}
                                </p>
                            ) : (
...
export default CinematographyValidator;