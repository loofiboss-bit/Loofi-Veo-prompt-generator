...
import { diffService, type DiffResult, type StructuredDiff } from '../services/diffService';
...
const DiffViewer: React.FC<DiffViewerProps> = ({ entry1, entry2, onClose, onRestore }) => {
    ...
    const renderDiffLine = (change: any, index: number) => {
        const bgColor =
            change.type === 'add' ? 'bg-green-900/30' :
                change.type === 'remove' ? 'bg-red-900/30' :
                    change.type === 'modify' ? 'bg-yellow-900/30' :
                        'bg-transparent';

        const borderColor =
            change.type === 'add' ? 'border-l-2 border-green-500' :
                change.type === 'remove' ? 'border-l-2 border-red-500' :
                    change.type === 'modify' ? 'border-l-2 border-yellow-500' :
                        '';
                        ...
                        <div key={key} className="bg-slate-800/50 rounded-lg p-3 border border-slate-700">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="flex-1 bg-red-900/20 border border-red-900/30 rounded px-2 py-1">
                                    <span className="text-red-400 text-xs font-mono">{String(change.old) || '(empty)'}</span>
                                </div>
                                ...
                                <div className="flex-1 bg-green-900/20 border border-green-900/30 rounded px-2 py-1">
                                    <span className="text-green-400 text-xs font-mono">{String(change.new) || '(empty)'}</span>
                                </div>
                            </div>
                        </div>
                    );
                ...
                {removed.length > 0 && (
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {removed.map((tag, i) => (
                                <span key={i} className="px-2 py-1 bg-red-900/20 border border-red-900/30 rounded text-xs text-red-400">
                                    - {tag}
                                </span>
                            ))}
                        </div>
                ...
                <header className="flex items-center justify-between p-5 border-b border-slate-700 flex-shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 rounded-lg border border-slate-700">
                            ...
                            <span className={`text-sm font-bold ${getSimilarityColor(diff.prompt.summary.similarity)}`}>
                ...
                <div className="flex items-center gap-4 px-5 py-3 bg-slate-800/30 border-b border-slate-700/50">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-400">
                            <span className="font-semibold text-green-400">{diff.prompt.summary.additions}</span> additions
                        </span>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-red-500"></div>
                        <span className="text-xs text-slate-400">
                            <span className="font-semibold text-red-400">{diff.prompt.summary.deletions}</span> deletions
                        </span>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-yellow-500"></div>
                        <span className="text-xs text-slate-400">
                            <span className="font-semibold text-yellow-400">{diff.prompt.summary.modifications}</span> modifications
                        </span>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-slate-500"></div>
                        <span className="text-xs text-slate-400">
                            <span className="font-semibold text-slate-300">{diff.prompt.summary.unchanged}</span> unchanged
                        </span>
                    </div>
                </div>
