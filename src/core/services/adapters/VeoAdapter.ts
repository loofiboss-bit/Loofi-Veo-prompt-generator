...
export class VeoAdapter implements VideoModelAdapter {
  ...
  getEnhancements(key: keyof PromptState, value: string): string {
    ...
    switch (key) {
      ...
      case 'lensType':
        if (value.includes('Macro'))
          return ' (Optics: Extremely shallow depth of field, sharp focus plane, smooth creamy bokeh)';
        if (value.includes('Anamorphic'))
          return ' (Optics: Oval bokeh, horizontal lens flares, cinematic aspect ratio)';
        break;
    }
    return '';
  }
  ...
  buildPrompt(state: PromptState, variables: Record<string, string>): string {
    ...
    if (iState.globalStyle && iState.globalStyle.isLocked && iState.globalStyle.description) {
      segments.push(
        `PROJECT VISUAL STYLE (STRICT): ${iState.globalStyle.description} (Consistency Strength: ${iState.globalStyle.strength}%). Ensure this shot matches this specific visual aesthetic exactly.`,
      );
    }
    ...
    if (iState.characterVisualDNA && iState.characterVisualDNA.trim()) {
      ...
      if (
        (iState.characterArchetype && iState.characterArchetype !== 'Any') ||
        (iState.characterGender && iState.characterGender !== 'Any')
      ) {
        const charParts = [];
    ...
    if (iState.lensType)
      camera.push(iState.lensType + this.getEnhancements('lensType', iState.lensType));
    ...
    if (iState.visualEffect && iState.visualEffect !== 'None')
      specs.push(
        `Effect: ${iState.visualEffect}` +
          this.getEnhancements('visualEffect', iState.visualEffect),
      );
