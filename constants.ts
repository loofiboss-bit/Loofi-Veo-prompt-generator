...
export const getModelOptions = (lang: Language): SelectOption[] => {
  const options: { [key: string]: { [lang in Language]: string } } = {
    'gemini-3-pro-preview': { en: 'Pro (Advanced Reasoning)', sv: 'Pro (Avancerat Resonemang)', es: 'Pro (Razonamiento Avanzado)', fr: 'Pro (Raisonnement Avancé)', de: 'Pro (Fortgeschrittenes Denken)' },
...
export const getLightingStyles = (lang: Language): SelectOption[] => {
    const styles: { [key: string]: { [lang in Language]: string } } = {
        ...
        'Soft, diffused light': { en: 'Soft, diffused light (overcast)', sv: 'Mjukt, spritt ljus (mulet)', es: 'Luz solar dura y difusa (nublado)', fr: 'Lumière douce et diffuse (couvert)', de: 'Weiches, diffuses Licht (bewölkt)' },
    };
    return Object.keys(styles).map(key => ({ value: key, label: styles[key][lang] }));
};
...
export const MUSIC_GENRES = "Rock, Pop, Jazz, Blues, Classical, Hip-Hop, Reggae, Country, Electronic, Metal, Punk, Folk, R&B, Soul, Funk, Disco, House, Techno, Trance, Dubstep, Drum and Bass, Indie, Alternative, Latin, K-Pop, J-Pop, Afrobeat, Gospel, Blues, Ska, Ambient, Lo-fi, Synth-Pop, Soundtrack, World Music, Experimental";