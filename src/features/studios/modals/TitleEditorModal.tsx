...
const TitleEditorModal: React.FC<TitleEditorModalProps> = ({ isOpen, onClose, shot, onSave }) => {
    ...
    const activeVideoSrc = (shot.takes && typeof shot.selectedTakeIndex === 'number' && shot.takes[shot.selectedTakeIndex]) 
    ...
    const updateActive = (updates: Partial<TextOverlay> | Partial<TextOverlay['style']> | Partial<TextOverlay['position']>) => {
        if (!activeId) return;
        ...
        setOverlays(prev => prev.map(o => {
            ...
            if ('fontSize' in updates || 'color' in updates || 'backgroundColor' in updates || 'backgroundOpacity' in updates) {
                ...
                <div className="flex-grow flex overflow-hidden">
                    ...
                    <div className="flex-1 bg-black relative flex items-center justify-center overflow-hidden">
                        <div className="relative max-h-full max-w-full aspect-video shadow-2xl bg-slate-900">
                            {/* Video Base */}
                            {activeVideoSrc ? (
                                <video src={activeVideoSrc} className="w-full h-full object-contain" muted loop autoPlay />
                            ) : shot.conceptImageUrl ? (
                                <img src={shot.conceptImageUrl} className="w-full h-full object-contain opacity-50" alt="Ref" />
                            ) : (
                        ...
                        <div className="flex-grow overflow-y-auto p-4 space-y-6">
                            ...
                            <div className="space-y-2 mb-6">
                                {overlays.map(overlay => (
                                    ...
                                    >
                                        <span className="text-xs text-slate-200 truncate max-w-[150px]">{overlay.text}</span>
                                        <button 
                            ...
                            {activeOverlay ? (
                                <div className="space-y-5 animate-fade-in-up">
                                    ...
                                    <div className="space-y-3">
                                        <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Style</h4>
                                        ...
                                        <RangeInput
                                            ...
                                            onChange={(e) => updateActive({ backgroundOpacity: parseInt(e.target.value) / 100 })}
                                            min={0}
                                            max={100}
                                        />
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Position</h4>
                                        <RangeInput
                                            label="X Position (%)"
                                            name="posX"
                                    ...
                                    <div className="space-y-3">
                                        <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Timing (Seconds)</h4>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
