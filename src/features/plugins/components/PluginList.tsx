                ...
                {plugins.map((plugin) => (
                    ...
                    >
                        <div className="flex items-center gap-4">
                            <div className={`p-3 rounded-lg ${plugin.state === 'active' ? 'bg-fuchsia-900/20 text-fuchsia-400' : 'bg-slate-700/30 text-slate-500'}`}>
                                <Icon name="layers" className="w-6 h-6" />
                            </div>
                            <div>
                                <h4 className="font-bold text-slate-200 flex items-center gap-2">
                                    {plugin.manifest.name}
                                    <span className="text-xs bg-slate-700 px-1.5 py-0.5 rounded text-slate-400 font-mono">v{plugin.manifest.version}</span>
                                </h4>
                        ...
                        <div className="flex items-center gap-3">
                            <div className={`px-2 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${plugin.state === 'active'
                            ...
                            <button
                                ...
                                className={`w-10 h-6 rounded-full relative transition-colors ${plugin.state === 'active' ? 'bg-fuchsia-600' : 'bg-slate-600'
                                    }`}
                            >
                                <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${plugin.state === 'active' ? 'translate-x-4' : 'translate-x-0'
                                    }`} />
                            </button>
                        </div>
                    </div>
