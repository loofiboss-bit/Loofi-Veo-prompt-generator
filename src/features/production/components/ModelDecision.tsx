...
export function ModelDecision({ request }: { request: VeoGenerationRequest }) {
  ...
  return (
    <aside
      ...
      className="mt-3 rounded-md border border-blue-500/30 bg-blue-950/20 p-3 text-xs"
    >
      <div className="flex flex-wrap items-center gap-2">
        <strong className="text-blue-100">{model.displayName}</strong>
        <span className="rounded bg-blue-500/20 px-2 py-0.5 uppercase text-blue-200">
          {model.lifecycle}
        </span>
        <span className="text-emerald-300">
          {maximumCost.maximumChargeUsd === null
            ? 'maximum unavailable'
            : `${maximumCost.confidence} maximum $${maximumCost.maximumChargeUsd.toFixed(2)}`}
        </span>
      </div>
      ...
      <p className="mt-1 text-slate-500">
        Source: {maximumCost.source.catalogSource} · verified {maximumCost.source.verifiedDate}
      </p>
      <p className="mt-1 text-slate-500">
