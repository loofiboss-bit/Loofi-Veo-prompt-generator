...
import { ExamplePrompt } from '@core/types';
...
const ExamplesCarousel: React.FC<ExamplesCarouselProps> = ({
  ...
  useEffect(() => {
    ...
    if (examples.length > 0) {
      ...
      setLiveRegionText(
        `Showing example ${currentIndex + 1} of ${examples.length}: ${currentExample.title}`,
      );
    }
    ...
    <div className="bg-slate-900/60 backdrop-blur-lg p-4 sm:p-6 rounded-2xl shadow-xl border border-slate-800 relative">
      ...
      <div className="flex items-center justify-between mt-4">
        <div className="flex items-center space-x-2">
          <button
            onClick={handlePrev}
            className="p-2 bg-slate-800/50 hover:bg-slate-700 rounded-full text-slate-300 transition-colors"
