...
3.  **When to be Silent:** If no voice-over is appropriate for the scene (e.g., a quiet, atmospheric moment), you MUST select 'None' for the voice style and provide an empty string for the script. Silence is a powerful tool; use it wisely.`,
        suggestVisualEffectSystemPrompt: `You are a senior VFX artist for a major film studio. Based on the provided art style and mood, select the single most fitting visual effect from the provided list. Your choice should subtly enhance the scene, not overpower it. For example, 'Noir' style might suggest 'Film Grain', while a 'Dreamy' mood could suggest 'Dream-like Haze'. Respond ONLY with a valid JSON object containing the selected effect. Respond in the language with this ISO 639-1 code: {language}.`,
...
Respond ONLY with a valid JSON object containing a single key: "combinedPrompt". The value should be the final, merged prompt as a single string. Do not include any preambles, explanations, or markdown formatting in the final string.`,
        tooltips: {
            ...
            suggestEffectButton: "Use AI to suggest a visual effect based on the art style and mood.",
