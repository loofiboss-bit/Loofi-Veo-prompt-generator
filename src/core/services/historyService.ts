...
class HistoryService {
  ...
  async getEntries(filter?: HistoryFilter): Promise<HistoryEntry[]> {
    try {
      ...
      const historyKeys = allKeys.filter((key) => String(key).startsWith(this.HISTORY_PREFIX));
   ...
   */
  async exportHistory(format: 'json' | 'csv' = 'json', filter?: HistoryFilter): Promise<string> {
    try {
