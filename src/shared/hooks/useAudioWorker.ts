...
export const useAudioWorker = () => {
  ...
  useEffect(() => {
    ...
    workerRef.current = new Worker(
      new URL('../../infrastructure/workers/audioProcessor.worker.ts', import.meta.url),
      {
        type: 'module',
      },
    );
      ...
      offlineCtx
        ...
        .then((renderedBuffer) => {
          ...
          workerRef.current?.postMessage(
            {
              type: 'DETECT_BEATS',
              payload: { channelData: dataCopy, sampleRate: audioBuffer.sampleRate },
            },
            [dataCopy.buffer],
          );
        })
  ...
  const analyzeSilence = useCallback(
    async (
  ...
  const generateWaveform = useCallback(
    async (audioBuffer: AudioBuffer, samples: number = 200): Promise<Float32Array> => {
      return new Promise((resolve, reject) => {
        if (!workerRef.current) return reject('Worker not initialized');
