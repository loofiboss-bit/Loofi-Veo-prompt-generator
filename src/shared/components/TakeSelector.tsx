        ...
        <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/50 mb-4">
            <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-slate-500 bg-slate-800 px-1.5 rounded-full">{takes.length}</span>
                </div>
                <span className="text-[9px] font-mono text-slate-600 truncate max-w-[100px]" title={activeAsset.takeGroupId}>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar snap-x">
                {takes.map((take) => {
                    ...
                    return (
                        ...
                        >
                            {take.type === 'video' ? (
                                <video src={take.proxyUrl || take.url} className="w-full h-full object-cover pointer-events-none" />
                            ) : (
                                <img src={take.url} className="w-full h-full object-cover" alt={`Take ${take.takeNumber}`} />
                            )}
